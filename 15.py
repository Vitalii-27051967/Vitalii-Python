...
class My15:
    ...
    def _generate_15(self):
        my_15 = []
        all_values = [str(i) for i in range(1, 16)] + [""]  # ряд чисел от 1 до 15 и ""
        random.shuffle(all_values)                          # перемешивает последовательность
        for row_number in range(4):
            my_15.append(all_values[4 * row_number: 4 * (row_number + 1)])  # создание списка из 4-х списков по 4 эл.
            print("Переменная my_15", my_15)
        return my_15

    def print_15(self):         # печатает строки одну под другой
