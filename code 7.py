...
# Вариант 2. Генератор ############
print("Задание 1. Вариант 1. Генератор", end="\n\n")
my_list_2 = []
new_list = [my_list_2.append(random.randint(1, 100)) for num_1 in range(0, 20)]
print(my_list_2, end="\n\n")
...
print("Ключи, которые есть в обоих словарях", is_in, end="\n\n")

non_in_1 = sorted(arl_1.difference(arl_2))      # Список ключей, которые есть только в первом словаре
print("Ключи, которые есть только в первом словаре", non_in_1, end="\n\n")

my_dict_3 = {}          # новый словарь из значений, которые есть в первом, но нет во втором словаре
for value in non_in_1:
    my_dict_3[value] = my_dict_1[value]
print("Список значений, которые есть только в первом словаре: ", my_dict_3, end="\n\n")
...
for key_com in arl:
    if my_dict_1.get(key_com, None):                # | Немного ломает мою логику мышления
        my_list_n.append(my_dict_1[key_com])        # |
    if my_dict_2.get(key_com, None):                # |
...
print("Объединённое значений словарей:", my_dict_4, sep="\n\n")
